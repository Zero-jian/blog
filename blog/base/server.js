...
const Label = require('../server/model/label');
...
const server = {
    ...
    labelSave(params, callback) {
        Label.find({ label: params.label }, (err, doc) => {
            if (err) {
                var result = {
                    status: data.status_err,
                    msg: 'err:' + err,
    ...
    write(params, callback) {
        Article.findOne({ 'articleId': params }, (err, doc) => {
            if (err) {
                var result = {
                    status: data.status_no,
                    msg: 'err:' + err,
    ...
    saveComment(params, callback) {
        ...
        if (params.id == 888888) {
            params.image = 'http://localhost:8080/static/logo.jpg';
            params.name = params.name + '(作者)';
        } else {
            params.image = 'http://localhost:8080/static/user.png';
        }
    ...
    replyComment(params, callback) {
        if (params.id == 888888) {
            params.msgImage = data.url + '/static/logo.jpg';
        } else {
            params.msgImage = data.url + '/static/user.png';
        }
        Comment.update({
